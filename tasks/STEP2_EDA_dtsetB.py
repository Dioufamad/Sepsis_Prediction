...
# NB : the previous command can give us the dimensions of the table but we prefer the df.info() command that is more informative
df0.info()
# """
...
# """
# + Remark 1 :
# the table dimensions are [761995 rows x 41 columns]
# + Remark 2 :
# - rows are the samples/observations/data points (761995).
...
df1 = df1.drop(['Unit1', 'Unit2'], axis=1) # [761995 rows x 39 columns]
...
df1_duplicate_rows.shape # (2735, 39) ie 2735 duplicates rows found
# + dropping the duplicate rows
df2 = df1.copy()
df2 = df2.drop_duplicates() # [759260 rows x 39 columns]
...
missing_data_al80 = missing_data[(missing_data.Percent >= 80)] # 26
missing_data_al80_share = (missing_data_al80.shape[0]) / (missing_data.shape[0])
missing_data_al80_perc = round((missing_data_al80_share*100), 2) # 66.67
...
missing_data_al15 = missing_data[(missing_data.Percent >= 15)] # 29
missing_data_al15_share = (missing_data_al15.shape[0]) / (missing_data.shape[0])
missing_data_al15_perc = round((missing_data_al15_share*100), 2) # 74.36
...
# + a group of vars missing almost all data :
missing_data_missalmostall_grp = missing_data[(missing_data.Percent > 92)] # 26
missalmostall_grp_list = missing_data_missalmostall_grp.index.values.tolist()
print("List of vars with % missing data > 92 has length", len(missalmostall_grp_list), "and is", missalmostall_grp_list)
# the list is :
# 25 out of the 26 lab values vars (the one var not registered here from the lab values vars is Glucose...)
# and 'EtCO2' from the group of vars "Vital Signs"
# - action : drop those 26 vars
# - situation report :
# the group of vars "Vital Signs" will have 1 out of 8 vars dropped ('EtCO2'), 7 vars to remain
# the group of vars "Lab values" will have 25 out of 26 vars dropped, and 1 out 26 vars to remain
...
# + a group largely missing data (65% to 78%) :
missing_data_largely_grp = missing_data[(missing_data.Percent > 65) & (missing_data.Percent < 78)] # 2
largely_grp_list = missing_data_largely_grp.index.values.tolist()
print("List of vars with % missing data > 65 and < 78 has length", len(largely_grp_list), "and is", largely_grp_list)
# the list is :
# 'Glucose' from the group of vars "Lab values" (~77.76% missing data),
# and 'Temp' from the group of vars "Vital Signs" (~65.98% missing data),
# - action 1 : drop the var Glucose (with its ~77.76% of missing data, it is close to the levels of missing data
# in the remaining of the group of vars "Lab values", that will also be dropped)
# - action 2 : for Temp (~65.98% missing data), 2 choices exist :
...
# - situation report :
# the group of vars "Vital Signs" will have 1 out of 8 vars dropped ('EtCO2'), the var Temp that can be kept or not, and 6 out of 8 vars to remain
# the group of vars "Lab values" will have 26 out of 26 vars dropped
# the group of vars "Demographics" have 2 out 6 vars already dropped ('Unit1', 'Unit2') so 4 out of 4 vars to remain
# > vars with missing data to keep : Temp if it is kept,

# + a group that equals or exceed the 15% of missing data threshold, up to a certain point (15% to 50%) :
missing_data_lessthanlargely_grp = missing_data[(missing_data.Percent >= 15) & (missing_data.Percent < 50)] # 1
lessthanlargely_grp_list = missing_data_lessthanlargely_grp.index.values.tolist()
print("List of vars with % missing data >= 15 and < 50 has length", len(lessthanlargely_grp_list), "and is", lessthanlargely_grp_list)
# the list is ['Resp'](~20.85% missing data) from the group of vars "Vital Signs".
# - action  : we choose to keep it.
# > Reason 1/3 : this var is part of the group of vars "Vital Signs", that is medically important to follow the patient state
# hence this var (as well as any var of this group) can be interesting to test out in a prediction of the patient state
# towards a medical event such as the sepsis onset we are trying to early predict.
# > Reason 2/3 : this var is part of a group of vars that does not show a trend of a lot of missing data (up to this point,
# from the group of vars "Vital Signs", only the var EtCO2 have to be dropped out of the 8 vars in the group). The var Resp
# is close to 5% away from the arbitrary 15% limit of missing data so it is not deviating a lot from the trend of the rest
# of its group (that will be most likely kept because under 15% missing data) so all the more reason to keep the var "Resp" also
# > Reason 3/3 (not necessary but an additional comforting argument):
# this EDA is part of a project where 2 hospitals (A and B) data have to be analysed for the same prediction problem.
# This dataset B is for the hospital B and there is a dataset A for hospital A.
# The dataset A has the same vars initially (before EDA, same features, same dep var) so an idea that can already be projected for
# future learning tasks is that, if after the EDA on each dataset, the 2 datasets have their remaining features space that
# are close (same remaining features or some of them in common), we could compare predictive models that are only different
# by the hospital data used or we can test models that pool the data from the 2 hospitals (e.g. multitask learning). So to push
# for such tests, we can keep any var that dont have too much missing data (more than 15% but still less than 50%) when this does favor,
# after EDA, the 2 datasets having their remaining features space being close.
# In the dataset A (the other hospital dataset), when it comes to the amount of missing data in the vars of the group of vars "Vital Signs",
# the observed trend was similar to what we have here and the var 'Resp' was kept there. So, here, in the dataset B,
# we push for keeping the var 'Resp', despite its ~20.85% missing data.
# - situation report :
# the group of vars "Vital Signs" will have 1 out of 8 vars dropped ('EtCO2'), the var Temp that can be kept or not, and 6 out of 8 vars to remain
# the group of vars "Lab values" will have 26 out of 26 vars dropped
# the group of vars "Demographics" have 2 out 6 vars already dropped ('Unit1', 'Unit2') so 4 out of 4 vars to remain
# > vars with missing data to keep : Temp if it is kept, 'Resp',
...
# the list is ['MAP', 'O2Sat', 'DBP', 'SBP', 'HR'] :
# its the remaining 5 out of 8 vars left from the group of vars "Vital Signs"
# (the other 3 are : EtCO2 that been dropped, Temp that can be kept or not, and Resp that will be kept)
# - initial point of view : we decide to keep all the 5 vars as they are vars that we would like to know how they participate in the prediction.
# - a deeper point of view : for DBP and SBP (resp. ~13.63% and ~13.61% missing data) :
...
# NB : Although, if availability is judged by % of missing data, MAP has slightly more missing data here (~ 14.44%) in
# comparison to DBP and SBP (resp. ~13.63% and ~13.61%) but the 3 values are quite close (~1% difference) so this does
# not keep us from maintaining MAP and dropping DBP and SBP
...
# the group of vars "Lab values" will have 26 out of 26 vars dropped
# the group of vars "Demographics" have 2 out 6 vars already dropped ('Unit1', 'Unit2') so 4 out of 4 vars to remain
# > vars with missing data to keep : Temp if it is kept, 'Resp', 'MAP', 'O2Sat', 'HR',
...
missing_data_nomiss_grp = missing_data[(missing_data.Percent == 0)] # 5
...
# the list is ['Age', 'Gender', 'HospAdmTime', 'ICULOS', 'SepsisLabel'] :
# its the remaining 4 vars from the group of vars "Demographics" + the dep var
...
# + Remark about the vars related to the length of stay of the patient :
# 'HospAdmTime' and 'ICULOS' might be essential to computing later on the utility metric so its extremely important to keep both of them.
# Here, they dont have missing values but in a case where there would be missing data in these 2 vars, we would choose to drop the rows/obs with missing data.
...
# NB : an additional option is to make a version of the dataset where, in addition to keep the var Temp, we keep also the
# 26 vars in the group "Laboratory values" to see their importance in the prediction. But this has 2 issues :
...
# df2_aft_initial_cleaning = df2 with ['EtCO2', 'DBP', 'SBP'] cols dropped, ['Resp', 'MAP', 'O2Sat', 'HR'] rows with missing data dropped
# - making df3
...
df2_aft_initial_cleaning = df2.copy() # [759260 rows x 39 columns]
df2_aft_initial_cleaning = df2_aft_initial_cleaning.drop(['EtCO2', 'DBP', 'SBP'], axis=1) # [759260 rows x 36 columns]
df2_aft_initial_cleaning = df2_aft_initial_cleaning.dropna(subset=['Resp', 'MAP', 'O2Sat', 'HR']) # [575642 rows x 36 columns] ie (183618 rows dropped)

# + making the list of 26 lab values vars to drop
# > missalmostall_grp_list contained all the vars with % missing data > 92 and corresponded to the 25 out of the 26 lab values vars
# (the one var not registered here from the lab values vars is 'Glucose'...) and 'EtCO2' from the group of vars "Vital Signs"
# > largely_grp_list contained all the vars with % missing data > 65 and < 78 and corresponded to ['Glucose','Temp']
list_labvalues_vars_todrop = missalmostall_grp_list.copy() # we use copy() and not just affecting to a new variable to avoid the remove function from removing also content from the original list
list_labvalues_vars_todrop.remove('EtCO2')
list_labvalues_vars_todrop.append(largely_grp_list[0]) # the 26 lab values vars are accounted for here

# + making df3
df3 = df2_aft_initial_cleaning.copy() # [575642 rows x 36 columns]
df3 = df3.drop(list_labvalues_vars_todrop, axis=1) # [575642 rows x 10 columns]
df3 = df3.drop(['Temp'], axis=1) # [575642 rows x 9 columns]
df3.isnull().sum().max() # check to see if no missing data left (output is 0 if no missing data left)
# + making df3_wTemp
df3_wTemp = df2_aft_initial_cleaning.copy() # [575642 rows x 36 columns]
df3_wTemp = df3_wTemp.drop(list_labvalues_vars_todrop, axis=1) # [575642 rows x 10 columns]
df3_wTemp = df3_wTemp.dropna(subset=['Temp']) # [227537 rows x 10 columns] ie (348105 rows dropped)
...
# df_wLabVals_wTemp = df2_aft_initial_cleaning.copy() # [575642 rows x 36 columns]
# df_wLabVals_wTemp = df2_aft_initial_cleaning.dropna(subset=['Temp']) # [227537 rows x 36 columns]
# df_wLabVals_wTemp = df_wLabVals_wTemp.dropna(subset=list_labvalues_vars_todrop) # Empty DataFrame
# # NB : There is no obs in df2_aft_initial_cleaning with a value in Temp and in all the 26 lab values vars
# # NB : df_wLabVals_wTemp even if pursued does not give a dataset so no learning task possible with the 26 lab values vars and Temp (under the condition of not imputing values)
...
df3_unscaled = df3.copy() # [575642 rows x 9 columns]
df3_wTemp_unscaled = df3_wTemp.copy() # [227537 rows x 10 columns]
...
df3_scaled = fts_scaling(df3, dep_var) # [575642 rows x 9 columns]
df3_wTemp_scaled = fts_scaling(df3_wTemp, dep_var) # [227537 rows x 10 columns]


print("# ---> Handling outliers 2/2 : the actual (first) removal of outliers")
# + making copies of our datasets to apply changes on them
# - for unscaled dfs
df4_unscaled = df3_unscaled.copy() # [575642 rows x 9 columns]
df4_wTemp_unscaled = df3_wTemp_unscaled.copy() # [227537 rows x 10 columns]
# - for scaled dfs
df4 = df3_scaled.copy() # [575642 rows x 9 columns]
df4_wTemp = df3_wTemp_scaled.copy() # [227537 rows x 10 columns]
...
# """
# ------Detecting outliers for a dataset------
# - Detecting outliers for the var HR and this is var 1 out of 7
#     num total obs is 575642 and num obs to remove is 6010 ie num obs to remain is 569632
# - Detecting outliers for the var O2Sat and this is var 2 out of 7
#     num total obs is 575642 and num obs to remove is 18383 ie num obs to remain is 557259
# - Detecting outliers for the var MAP and this is var 3 out of 7
#     num total obs is 575642 and num obs to remove is 9888 ie num obs to remain is 565754
# - Detecting outliers for the var Resp and this is var 4 out of 7
#     num total obs is 575642 and num obs to remove is 21584 ie num obs to remain is 554058
# - Detecting outliers for the var Age and this is var 5 out of 7
#     num total obs is 575642 and num obs to remove is 557 ie num obs to remain is 575085
# - Detecting outliers for the var HospAdmTime and this is var 6 out of 7
#     num total obs is 575642 and num obs to remove is 65796 ie num obs to remain is 509846
# - Detecting outliers for the var ICULOS and this is var 7 out of 7
#     num total obs is 575642 and num obs to remove is 27009 ie num obs to remain is 548633
# ------Removing the outliers detected------
# - Removed a total of outliers of 131384 out of 575642 obs ie 22.82 %
# - Remaining num total obs is 444258 out of 575642 obs ie 77.18 %
# """
...
#     num total obs is 227537 and num obs to remove is 2261 ie num obs to remain is 225276
# - Detecting outliers for the var O2Sat and this is var 2 out of 8
#     num total obs is 227537 and num obs to remove is 2224 ie num obs to remain is 225313
# - Detecting outliers for the var Temp and this is var 3 out of 8
#     num total obs is 227537 and num obs to remove is 4356 ie num obs to remain is 223181
# - Detecting outliers for the var MAP and this is var 4 out of 8
#     num total obs is 227537 and num obs to remove is 4546 ie num obs to remain is 222991
# - Detecting outliers for the var Resp and this is var 5 out of 8
#     num total obs is 227537 and num obs to remove is 9669 ie num obs to remain is 217868
# - Detecting outliers for the var Age and this is var 6 out of 8
#     num total obs is 227537 and num obs to remove is 3271 ie num obs to remain is 224266
# - Detecting outliers for the var HospAdmTime and this is var 7 out of 8
#     num total obs is 227537 and num obs to remove is 17895 ie num obs to remain is 209642
# - Detecting outliers for the var ICULOS and this is var 8 out of 8
#     num total obs is 227537 and num obs to remove is 11308 ie num obs to remain is 216229
# ------Removing the outliers detected------
# - Removed a total of outliers of 48101 out of 227537 obs ie 21.14 %
# - Remaining num total obs is 179436 out of 227537 obs ie 78.86 %
# """
...
# """
# - Num obs : class 0 has 440270 and class 1 has 3988
# - % obs in class : count class 0 accounts for 99.1 and count class 1 accounts for 0.9
# - ie a ratio classSup over classInf of :  110.0
# """
# - imbalance summary for df4_wTemp
imbalance_summary(df4_wTemp,dep_var)
# """
# - Num obs : class 0 has 177895 and class 1 has 1541
# - % obs in class : count class 0 accounts for 99.14 and count class 1 accounts for 0.86
# - ie a ratio classSup over classInf of :  115.0
# """
# - Remark 1 :
# both datasets show a strong imbalance, with the size of the class 0 being at least 110 times the size of the class 1, that we are trying to predict.
...
plt.title("Number of patients for each category of SepsisLabel (dtsetB, var Temp dropped)")
...
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_UA_Case1_1CatVar_theDepVar.png"
...
plt.figure(figsize=(7,5))
graph = sns.barplot(counts_by_cat.index, counts_by_cat.values, alpha=0.8)
plt.title("Number of patients for each category of Gender (dtsetB, var Temp dropped)")
plt.ylabel('Count', fontsize=12)
plt.xlabel('Gender', fontsize=12)
for p in graph.patches:
        graph.annotate('{:.0f}'.format(p.get_height()), (p.get_x()+0.3, p.get_height()),
                       ha='center', va='center', color='black', xytext = (25, 5), textcoords = 'offset points')
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_UA_Case1_1CatVar_Gender.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
...
plt.title("Distribution of the HR variable (dtsetB, var Temp dropped)")
plt.ylabel('Count', fontsize=12)
plt.xlabel('HR', fontsize=12)
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_UA_Case2_1ContVar_HR.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
...
plt.title("Distribution of the Temp variable (dtsetB, var Temp kept)")
plt.ylabel('Count', fontsize=12)
plt.xlabel('Temp', fontsize=12)
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_UA_Case2_1ContVar_Temp.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
...
plt.title("Scatter plot of the variables O2Sat and Resp (dtsetB, var Temp dropped)")
plt.ylabel('O2Sat', fontsize=12)
plt.xlabel('Resp', fontsize=12)
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_BA_Case1_2ContVars_O2Sat_Resp.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
# for df4_unscaled
...
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_BA_Case1_2ContVars_O2Sat_Resp_unscaled_values.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
...
plt.title("Boxplot of the variables HR and SepsisLabel (dtsetB, var Temp dropped)")
plt.ylabel('HR', fontsize=12)
plt.xlabel('SepsisLabel', fontsize=12)
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_BA_Case2_1ContVar1CatVar_HR_theDepVar.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
# """
...
# ***: 1.00e-04 < p <= 1.00e-03
# ****: p <= 1.00e-04
# 0 v.s. 1: Mann-Whitney-Wilcoxon test two-sided with Bonferroni correction, P_val=3.675e-93 U_stat=7.129e+08
# """
# - Remark 1 :  some outliers seem to exist (e.g. outside of upper whisker of SepsisLabel 0 boxplot, outside of lower whisker of SepsisLabel 1 boxplot)
# This can also be observed in a pairplot during multivariate analysis that implicates these vars.
# - Remark 2 : the 2 two groups of the dep var are differenciated by the HR var (P_val=3.675e-93 ie <= 1.00e-04 ie **** significance)
...
our_annotations_to_add = add_stat_annotation(ax, data=data, x=var2, y=var1, order=order,
                                             ...
                                             loc='inside', verbose=2)
our_annotations_to_add
plt.title("Boxplot of the variables Temp and SepsisLabel (dtsetB, var Temp kept)")
plt.ylabel('Temp', fontsize=12)
plt.xlabel('SepsisLabel', fontsize=12)
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_BA_Case2_1ContVar1CatVar_Temp_theDepVar.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
# """
...
# ***: 1.00e-04 < p <= 1.00e-03
# ****: p <= 1.00e-04
# 0 v.s. 1: Mann-Whitney-Wilcoxon test two-sided with Bonferroni correction, P_val=7.541e-26 U_stat=1.158e+08
# """
# - Remark 1 :  no outliers observed
# - Remark 2 : the 2 two groups of the dep var are differenciated by the Temp var (P_val=7.541e-26 ie <= 1.00e-04 ie **** significance)
...
our_annotations_to_add = add_stat_annotation(ax, data=data, x=var2, y=var1,  hue=var3,order=order,
                                             ...
                                             loc='inside', verbose=2)
our_annotations_to_add
plt.title("Boxplot of the variables HR and SepsisLabel, for each Gender (dtsetB, var Temp dropped)")
plt.ylabel('HR (beats per minute)', fontsize=12)
plt.xlabel('SepsisLabel', fontsize=12)
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_BA_Case3_2CatVars1ContVar_Gender_theDepVar_HR.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
# """
...
# ***: 1.00e-04 < p <= 1.00e-03
# ****: p <= 1.00e-04
# 1_Male v.s. 1_Female: Mann-Whitney-Wilcoxon test two-sided with Bonferroni correction, P_val=1.000e+00 U_stat=1.950e+06
# 0_Male v.s. 0_Female: Mann-Whitney-Wilcoxon test two-sided with Bonferroni correction, P_val=2.292e-72 U_stat=2.334e+10
# 0_Male v.s. 1_Male: Mann-Whitney-Wilcoxon test two-sided with Bonferroni correction, P_val=3.610e-60 U_stat=2.097e+08
# 0_Female v.s. 1_Female: Mann-Whitney-Wilcoxon test two-sided with Bonferroni correction, P_val=6.952e-35 U_stat=1.490e+08
# """
# - Remark 1 :  some outliers seem to exist (e.g. outside of both whiskers of the 2 boxplots of Male and Female for SepsisLabel 0)
# - Remark 2 : a) the HR var differentiates significantly the two genders in SepsisLabel 0 but not in SepsisLabel 1.
...
our_annotations_to_add = add_stat_annotation(ax, data=data, x=var2, y=var1, hue=var3, order=order,
                                             ...
                                             loc='inside', verbose=2)
our_annotations_to_add
plt.title("Boxplot of the variables Temp and SepsisLabel, for each Gender (dtsetB, var Temp kept)")
plt.ylabel('Temp (Deg C)', fontsize=12)
plt.xlabel('SepsisLabel', fontsize=12)
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_BA_Case3_2CatVars1ContVar_Gender_theDepVar_Temp.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
# """
...
# ***: 1.00e-04 < p <= 1.00e-03
# ****: p <= 1.00e-04
# 1_Male v.s. 1_Female: Mann-Whitney-Wilcoxon test two-sided with Bonferroni correction, P_val=1.000e+00 U_stat=2.976e+05
# 0_Male v.s. 0_Female: Mann-Whitney-Wilcoxon test two-sided with Bonferroni correction, P_val=8.461e-100 U_stat=4.104e+09
# 0_Male v.s. 1_Male: Mann-Whitney-Wilcoxon test two-sided with Bonferroni correction, P_val=3.332e-12 U_stat=3.896e+07
# 0_Female v.s. 1_Female: Mann-Whitney-Wilcoxon test two-sided with Bonferroni correction, P_val=9.939e-15 U_stat=2.035e+07
# """
# - Remark 1 :  some outliers seem to exist (e.g. outside of both whiskers of the boxplot for "Females with SepsisLabel 0")
# - Remark 2 : a) the Temp var differentiates significantly the two genders in SepsisLabel 0 but not in SepsisLabel 1.
...
plt.title("Heatmap showing the correlations between all remaining variables (dtsetB, var Temp dropped)")
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_MA_Case1_PairwiseCorrelationsHeatmap_VarTempDropped.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
# - Remark 1 :
...
# HR, MAP, ICULOS, HospAdmTime, Resp, O2Sat, Gender, and Age (different order in comparison to hospital A)
# We decide that those we will study more are vars with abs corr with dep var > 0.01 :
# HR, MAP, ICULOS, HospAdmTime. (3/4 similar in comparison to hospital A)
# - Remark 4 :
...
# ### the corr between HospAdmTime and MAP, the corr between ICULOS and Resp, the corr between ICULOS and HR, the corr between ICULOS and MAP,
...
# > 1 corr related to the length of stay :
# ### the corr between HospAdmTime and HR :
# can be explained by a shorter HospAdmTime (ie the patient being transferred faster in an ICU), translating into
# a better care hence measurables of the patient being better like a higher HR
...
# - Remark 5 : overall similarities/differences comparison to hospital A)
# > the order of the variables for the abs corr to the dep is different but the majority of the most correlated fts to the dep var is the same
# > a lot of the pairwise corr between fts are common with those found for hospital A
...
f, ax  = plt.subplots(figsize=(12, 9))
graph = sns.heatmap(final_corrmat, mask=mask_used, cmap="bwr", annot=True, square=True)
plt.title("Heatmap showing the correlations between all remaining variables (dtsetB, var Temp kept)")
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_MA_Case1_PairwiseCorrelationsHeatmap_VarTempKept.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
...
# HR, ICULOS, Temp, MAP, HospAdmTime, Resp, Age, Gender, O2Sat (different order in comparison to hospital A)
# We decide that those we will study more are vars with abs corr with dep var > 0.01 :
# HR, ICULOS, Temp, MAP, HospAdmTime (5/6 similar in comparison to hospital A)
# Remark 3 : the highest pairwise corrs distinguished, between Temp and fts, are in descending order :
# ### the positive corrs :
# with HR, Resp, ICULOS, and Gender (same group and order in comparison to hospital A)
# ### the negative corrs :
# with HospAdmTime, MAP, O2Sat and Age (same group but reverse order in comparison to hospital A)
...
# for df4
# # on vars with abs corr with dep var > 0.01 : HR, MAP, ICULOS, HospAdmTime.
sns.set()
# cols_selected = k_cols_in_desc_order_of_corr_with_dep_var_in_df4 # for all vars but in desc order of corr with the dep var
cols_selected = ['HR', 'MAP', 'ICULOS', 'HospAdmTime']
graph = sns.pairplot(df4[cols_selected], corner=True)
graph.fig.suptitle("Pairplot of sorted variables having correlation with dep var > 0.01 (dtsetB, var Temp dropped)", ha='center', va='center')
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_MA_Case2_PairwiseObservationsPairplot_VarTempDropped.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
# - Remark 1 :
...
# ... and HospAdmTime that is left skewed
...
# NB : This specificity about HospAdmTime was observed also in hospital A where HospAdmTime was positively correlated to the dep var
# in both versions of the dataset (Temp dropped and Temp kept), and this was shown on the pairplot of the dataset A version with Temp.
...
# for df4_wTemp (to have a look at the var Temp in particular)
# # on vars with abs corr with dep var > 0.01 : HR, ICULOS, Temp, MAP, HospAdmTime
sns.set()
...
cols_selected = ['HR', 'ICULOS', 'Temp', 'MAP', 'HospAdmTime']
sns.pairplot(df4_wTemp[cols_selected], corner=True)
graph.fig.suptitle("Pairplot of sorted variables having correlation with dep var > 0.01 (dtsetB, var Temp kept)", ha='center', va='center')
fullname_file_of_plot = output_directory+"/"+"dtsetB_STEP2_EDA_MA_Case2_PairwiseObservationsPairplot_VarTempKept.png"
plt.savefig(fullname_file_of_plot, bbox_inches='tight')
plt.show()
plt.close()
# - Remark 1 :
# same as the remark for the cluttered scatterplots in the pairplot of (dtsetB, var Temp dropped)
# - Remark 2 :
# same remark for ICULOS right skewness and HospAdmTime left skewness (dtsetB, var Temp dropped)
# - Remark 3 :
# same as the remark about "imbalance combined with correlation manifest in skewness" in the pairplot of (dtsetB, var Temp dropped)
...
# (df0 had [761995 rows x 41 columns])
...
# (df4 has [444258 rows x 9 columns] and df4_wTemp has [179436 rows x 10 columns])
...
# (df4_unscaled has [444258 rows x 9 columns] and df4_wTemp_unscaled has [179436 rows x 10 columns])
...
# df to save is df4 # [444258 rows x 9 columns]
fullname_file_of_df4 = output_directory+"/"+"dtsetB_forML_woTemp_V1.csv"
df4.to_csv(fullname_file_of_df4, header=True, index=False)
print("File dtsetB_forML_woTemp_V1 saved !")
# - version being saved : fit for learning task (cleaned, scaled) with var Temp
# df to save is df4_wTemp # [179436 rows x 10 columns]
fullname_file_of_df4_wTemp = output_directory+"/"+"dtsetB_forML_wTemp_V1.csv"
df4_wTemp.to_csv(fullname_file_of_df4_wTemp, header=True, index=False)
print("File dtsetB_forML_wTemp_V1 saved !")
# - version being saved : not fit for learning task but useful for some visualisations (cleaned, not scaled) without var Temp
# df to save is df4_unscaled # [444258 rows x 9 columns]
fullname_file_of_df4_unscaled = output_directory+"/"+"dtsetB_forViz_woTemp_V1.csv"
df4_unscaled.to_csv(fullname_file_of_df4_unscaled, header=True, index=False)
print("File dtsetB_forViz_woTemp_V1 saved !")
# - version being saved : not fit for learning task but useful for some visualisations (cleaned, not scaled) with var Temp
# df to save is df4_wTemp_unscaled # [179436 rows x 10 columns]
fullname_file_of_df4_wTemp_unscaled = output_directory+"/"+"dtsetB_forViz_wTemp_V1.csv"
df4_wTemp_unscaled.to_csv(fullname_file_of_df4_wTemp_unscaled, header=True, index=False)
print("File dtsetB_forViz_wTemp_V1 saved !")
